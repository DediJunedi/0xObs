...
def mainapp():
	...
	try: import akun
		...
		f.write('data = [\n{"0812xxx":"password"},\n{"628xxx":"password"},\n{"0812xxx":"password"}\n]')
		f.close()
		print("\nEdit File 'akun.py' Untuk Mengisi Data Akun Anda,\nHubungi t.me/@itsaoda Jika Ada Hal Yang Ingin Ditanyakan")
	finally:
		acc = akun.data

	from colorama import Fore as fff
	red = fff.RED
	g = fff.GREEN
	b = fff.BLUE
	y = fff.YELLOW
...
"Mozilla/5.0 (Linux, Android 9, Redmi 6) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/110.0.0.0 Mobile Safari/537.36"]
	...
	def idchecker(itsyou):
		...
		if (check.status_code == 200):
			if itsyou.replace(' ','') in check.text:
...
{y}{str(str(len(acc))+" Akun").rjust(42)}{w}''')
	...
	def menu2():
		...
		def multiedit():
			...
			for now,list in enumerate(acc):
				for no,passw in list.items():
					kuki = logedit(now,no,passw)
					listkuki.append(kuki)
			print("-"*42)
			print(f'> Total {len(listkuki)} Akun')
			aksi = input('\n  ([1]  Ultah Anak / [2] Ternak Lele)? : ')
			def editinanakkudongsay():
				aksiku = 'Ultah' if (aksi == '1') else 'Kawin'
				for akunke,kuki in enumerate(listkuki):
					ua = random.choice(ualist)
					print(f'\n {aksiku} akun ke-{akunke+1}\n  ->',end=' ')
					url = "https://www.generasimaju.co.id/klub-generasi-maju/edit"
					req = r.get(url,headers={"Content-Type":"application/x-www-form-urlencoded","User-Agent":ua,"Referer":"https://www.generasimaju.co.id/klub-generasi-maju/akun","Accept": "text/html,application/xhtml+xml,application/xml,q=0.9,image/avif,image/webp,image/apng,*/*,q=0.8,application/signed-exchange,v=b3,q=0.9","Accept-Encoding": "gzip, deflate","Accept-Language": "id-ID,id,q=0.9,en-US,q=0.8,en,q=0.7"},cookies=kuki)
					...
					if (aksi == '1'):
						...
						print(" Invalid Input (!)")
				input("\nTekan [Enter] Untuk Melanjutkan".center(42))
			editinanakkudongsay()
			'''
			semuadata = []
	...
	def postedit(kuki,data,boundID):
		...
		req = r.post(url,headers=head,cookies=kuki,data=data,allow_redirects=False)
		if req.status_code == 302:
			...
			for _ in range(3):
				print(f' \n{y} Retry...{w}',end="\r")
				postedit(kuki,data,boundID)
			print(f' {red} Gagal Terhubung Dengan Website {w}(!)')
	def editprofil(kuki):
		url = "https://www.generasimaju.co.id/klub-generasi-maju/edit"
		...
		if req.status_code == 200:
			...
			if not 'Redirecting to <a href="/klub-generasi-maju/login">/klub-generasi-maju/login</a>' in editpage:
				...
				address = get_between(editpage,f'<textarea name="address" id="alamat-profil" clasform-control" cols="5" rows="5" value="jalan" oninput="profilJalan()" placeholder="Isikan Alamat Bunda">','</textarea>')
				print('''
 {}• {}Profil Bunda {}:
  - Nama : {} {}
  - Tanggal Lahir : {}
...
'''.format(y,c,w,firstname,lastname,birthdate,msisdn,email,address))
				...
				for anakke in totalanak:
					...
					print("   - Nama Anak: {} | {}".format(nama if len(nama) > 1 else "<Tidak Bernama>",tl))
				print("\n  1) Edit Profil Bunda\n  2) Edit Profil Anak\n-----------------------")
				def pilihedit(kuki):
					editsiapa = input("  Pilih : ")
					if editsiapa == "1":
						boundID = ''.join([random.choice('ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmopqrstuvwxyz') for _ in range(16)])
						data = bodygen('bunda',editpage,totalanak,boundID)
						postedit(kuki,data,boundID)
					elif editsiapa == "2":
	...
	def bodygen(editType,editpage,totalanak,boundID):
		try: import confedit as config
		except:
			f = open("confedit.py","w")
			f.write('''
# Ultah ke ... tahun
ultah = 2
# Tambah anak / Ternak Lele
berapa = 1
tahun = 2020
bulan = 3
tanggal = 28
...
### CATATAN PENTING !! ##
# Jangan edit tulisan (ultah,berapa,tahun,bulan,tanggal)
# Edit di bagian value nya saja, misal mau ganti jumlah anak yang mau ditambah
# Harap untuk menaati aturan-aturan yang sudah disepakati
# Sekian terima gaji

''')
			f.close()
			print("\nFile config telah dibuat, jalankan ulang script ini untuk melanjutkan.\nHubungi t.me/@itsaoda Jika Ada Hal Yang Ingin Ditanyakan")
		finally:
			ultah = config.ultah
			# Tambah anak / Ternak Lele
			berapa = config.berapa
			tahun = config.tahun
			bulan = config.bulan
		...
		#Sedang Hamil dan Mempunyai Anak
		...
		for child in totalanak:
			...
			firstname = get_between(editpage,f'name="children[{child}][firstname]" value="','"')
			childdate = get_between(editpage,f'name="children[{child}][birthdate]" class="form-control tanggal-anak" value="','"')
		...
		if (editType == "ultah"):
			...
			while True:
				try:
					plusanak = berapa
					print(f" Menambah {plusanak} Anak ")
					if plusanak + plus <= 20: ##
						if plusanak + plus >= 10:
			...
			for x,_ in enumerate(range(plusanak)):
				#plus += x + 1
				plus = len(listdata) + x
				namaAnak = str(raname.getname()).split(' ')[0]
				print(f" Nama Anak ke {plus+1} -> {namaAnak}")
			...
			print(f'{y}* Kosongkan jika tidak ingin diubah{w}')
			dfirstname = input(f'{w} Nama Depan -> {g}')
			if not (len(dfirstname) == 0):
				firstname = dfirstsname

			dlastname = input(f'{w} Nama Belakang -> {g}')
			if not (len(dlastname) == 0):
				lastname = dlastname

			dbirthdate = input(f'{w} Tangal Lahir ({y}tahun-bulan-tgl{w}) -> {g}')
			if not (len(dbirthdate) == 0):
				birthdate = dbirthdate

			dmsisdninput = input(f'{w} Nomor ({y}{msisdn}{w}) -> {g}')
			if not (len(dmsisdninput) == 0):
				msisdninput = dmsisdninput

			demail = input(f'{w} Email ({y}{email}{w}) -> {g}')
			if not (len(demail) == 0):
				email = demail

			daddress = input(f'{w} Alamat ({y}{address}{w}) -> {g}')
			if not (len(daddress) == 0):
				address = daddress

			biasa = ''
			for index,key in enumerate(listdata):
				nolist = str(index)
				biasa  += updatedChild+nolist+'][childid]"\r\n\r\n'+str(key['childid'])+"\r\n"
				biasa  += updatedChild+nolist+'][firstname]"\r\n\r\n'+key['firstname']+"\r\n"
				biasa  += updatedChild+nolist+'][birthdate]"\r\n\r\n'+key['birthdate']+"\r\n"
				biasa  += updatedChild+nolist+'][childmilk]"\r\n\r\n'+str(key['childmilk'])+"\r\n"
			profilEdited = biasa
		...
		pregnanYN = f'------WebKitFormBoundary{boundID}\r\nContent-Disposition: form-data; name="pregnancyweekYN"\r\n\r\n{pregnancyweek}\r\n'
		endtag = f"------WebKitFormBoundary{boundID}--"
		ultah = profil + profilEdited + step2  + endtag
		kawin = profil + step1 + profilEdited + pregnanYN + step2  + endtag
		bunda = profil + step1 + step2 + endtag
		if (editType == 'ultah'):
			return ultah
		elif (editType == 'kawin'):
			return kawin
	...
	def cekanak(kuki,h):
		url = "https://www.generasimaju.co.id/klub-generasi-maju/edit"
		cekinfoedit = r.get(url,headers=h,cookies=kuki,allow_redirects=False)
		#input(cekinfoedit)
		#input(cekinfoedit.text)
		with open('log.html','w') as f:
			f.write(cekinfoedit.text)
			f.close()
	...
	def logprofil(nomer,passw):
		...
		if loginpost.status_code == 302:
			...
			loginget = r.get(url,headers=headers,cookies=loginpost.cookies)
			if '>Login Berhasil<' in loginget.text:
				...
				cekanak(loginpost.cookies,{"Content-Type":"application/x-www-form-urlencoded","User-Agent":ua,"Referer":"https://www.generasimaju.co.id/klub-generasi-maju/akun","Accept": "text/html,application/xhtml+xml,application/xml,q=0.9,image/avif,image/webp,image/apng,*/*,q=0.8,application/signed-exchange,v=b3,q=0.9","Accept-Encoding": "gzip, deflate","Accept-Language": "id-ID,id,q=0.9,en-US,q=0.8,en,q=0.7"})
			else:
	...
	idchecker(str(devbrand+hashlib.sha1(repr(hashlib.md5(repr(os.getlogin()).encode()).hexdigest()+devid).encode()).hexdigest()).replace(' ',''))
